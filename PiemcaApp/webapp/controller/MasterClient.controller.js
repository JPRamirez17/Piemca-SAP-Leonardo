sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	return BaseController.extend("com.sap.build.standard.piemca.controller.MasterClient", {
			...
			if (status) {
				...
				return new Promise(function(fnResolve, fnReject) {
					...
					if (bHasPendingChanges) {
						...
						var oNewEntityInstance = Utilities.getDefaultValuesForDetailClientCreate();

						oModel = this.getView().getModel();
						var oNewBindingContext = oModel.createEntry("ClientSet", {
							properties: oNewEntityInstance
						});

						this.doNavigate("DetailClientCreate", oNewBindingContext, fnResolve);
					}
				}.bind(this)).catch(function(err) {
					if (err !== undefined) {
						MessageBox.error(err.message);
		...
		_onObjectListItemPress: function(oEvent) {
			...
			return new Promise(function(fnResolve) {
				this.doNavigate("DetailClient", oBindingContext, fnResolve, "");
			}.bind(this)).catch(function(err) {
				if (err !== undefined) {
					MessageBox.error(err.message);
				}
		...
		_onSearchFieldLiveChange: function(oEvent) {
			...
			return new Promise(function(fnResolve) {
				...
				if (sQuery && sQuery.length > 0) {
					...
					aFilters.push(new sap.ui.model.Filter("Phone", sap.ui.model.FilterOperator.Contains, sQuery));

					aFilters.push(new sap.ui.model.Filter("Contact", sap.ui.model.FilterOperator.Contains, sQuery));
		...
		onInit: function() {
			...
			this.oRouter.getTarget("MasterClient").attachDisplay(jQuery.proxy(this.handleRouteMatched, this));
