sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	return BaseController.extend("com.sap.build.standard.piemca.controller.MasterCongress", {
			...
			if (status) {
				...
				return new Promise(function(fnResolve, fnReject) {
					...
					if (bHasPendingChanges) {
						...
						var oNewEntityInstance = Utilities.getDefaultValuesForDetailCongressEdit();

						oModel = this.getView().getModel();
						var oNewBindingContext = oModel.createEntry("CongressSet", {
							properties: oNewEntityInstance
						});

						this.doNavigate("DetailCongressEdit", oNewBindingContext, fnResolve);
					}
				}.bind(this)).catch(function(err) {
					if (err !== undefined) {
						MessageBox.error(err.message);
		...
		_onObjectListItemPress: function(oEvent) {
			...
			return new Promise(function(fnResolve) {
				this.doNavigate("DetailCongress", oBindingContext, fnResolve, "");
			}.bind(this)).catch(function(err) {
				if (err !== undefined) {
					MessageBox.error(err.message);
				}
		...
		_onSearchFieldLiveChange: function(oEvent) {
			var sControlId = "sap_List_Page_0-content-sap_m_ObjectList-1-poqilvzj25kr68rzxzxvnq5b3_S3";
			...
			return new Promise(function(fnResolve) {
				...
				if (sQuery && sQuery.length > 0) {
					...
					if (!isNaN(iQuery)) {
						aFilters.push(new sap.ui.model.Filter("Assistants", sap.ui.model.FilterOperator.EQ, sQuery));
					}
					...
					var iQuery = parseFloat(sQuery);
					if (!isNaN(iQuery)) {
						aFilters.push(new sap.ui.model.Filter("Cost", sap.ui.model.FilterOperator.EQ, sQuery));
					}

					aFilters.push(new sap.ui.model.Filter("City1", sap.ui.model.FilterOperator.Contains, sQuery));

					var iQuery = parseFloat(sQuery);
					if (!isNaN(iQuery)) {
						aFilters.push(new sap.ui.model.Filter("SurveyCount", sap.ui.model.FilterOperator.EQ, sQuery));
					}

					var iQuery = parseFloat(sQuery);
					if (!isNaN(iQuery)) {
						aFilters.push(new sap.ui.model.Filter("Effectiveness", sap.ui.model.FilterOperator.EQ, sQuery));
					}
		...
		_onButtonPress2: function(oEvent) {
			...
			return new Promise(function(fnResolve) {
				...
				this.doNavigate("DetailCongressCreate", oBindingContext, fnResolve, "");
			}.bind(this)).catch(function(err) {
				if (err !== undefined) {
					MessageBox.error(err.message);
				}
		...
		onInit: function() {
			...
			this.oRouter.getTarget("MasterCongress").attachDisplay(jQuery.proxy(this.handleRouteMatched, this));
		...
		onExit: function() {
			...
			var aControls = [{
				"controlId": "sap_List_Page_0-content-sap_m_ObjectList-1-poqilvzj25kr68rzxzxvnq5b3_S3",
				"groups": ["items"]
			}];
